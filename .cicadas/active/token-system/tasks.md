...
- [x] Add nullable `transactions.game_id` FK to `games` + `transactions_game_id_source_idx`; migration `0004` includes a backfill of historical rows from their reason strings. Builder-approved 2026-07-25 as the durable fix for the Partition 4 analytics coupling <!-- id: 95 -->
- [x] Thread `gameId` through `lib/ledger.ts`'s game-related insert sites (`writeTransaction`, `awardAchievement`, `settleTopScoreAward`) and their callers in `lib/spend.ts`, `lib/leaderboard.ts`, `lib/achievements.ts`. Content, login, and admin-adjustment writes leave it null <!-- id: 96 -->
- [x] Switch `lib/admin.ts` Analytics "most played" to join on `transactions.game_id` instead of a reconstructed reason string; test asserts the count survives a reason-wording change <!-- id: 97 -->

## Partition: feat/site-integration

- [x] Add `src/lib/tokenApi.ts` API-client module wrapping fetch calls to the backend, with graceful degradation when the backend is unreachable <!-- id: 80 -->
- [x] Add "Sign in with Google" control to the existing header, replaced by the balance pill when signed in, per UX Mock-Up 2 (`site-balance-pill.html`) <!-- id: 81 -->
- [x] Implement the balance pill's Loading / Populated states with the LED-pulse convention <!-- id: 82 -->
- [x] Implement the transient achievement/award toast (`aria-live="polite"`, ~3s auto-dismiss, fades instead of slides under `prefers-reduced-motion`) <!-- id: 83 -->
- [x] Wire `POST /api/spend` into the existing `Hub.register(...)` cartridge/cabinet start flow; block game start on insufficient balance <!-- id: 84 -->
- [x] Implement the `"Need {N} tokens"` inline veil for insufficient-balance attempts, without stealing keyboard focus from other games <!-- id: 85 -->
- [x] Wire `POST /api/scores/submit` into each existing game's score-save path <!-- id: 86 -->
- [x] Build the minimal Account surface: Discord account-link CTA (using the Discord provider stubbed in `feat/backend-foundation`), own transaction log view <!-- id: 87 -->
- [x] Build the Riddles/Tasks list on the Games Tab, calling `GET /api/content` / `POST /api/content/complete`; must render correctly in the expected-empty state at launch (no content seeded yet) per UX Riddles/Tasks List states <!-- id: 87a -->
- [x] Verify a simulated backend outage leaves core gameplay fully playable (click-to-wake, pause-on-click-away, fullscreen, Escape) with only the balance pill degraded <!-- id: 88 -->
- [x] Manual regression pass against the existing hub interaction model per approach.md Risks & Mitigations <!-- id: 89 -->
- [x] Accessibility check: keyboard navigation, `aria-live` toast behavior, reduced-motion, color contrast on new elements (WCAG 2.1 AA) <!-- id: 90 -->
- [x] Add an opt-in `Hub.setWakeGate()` hook so spend-before-play can decline a wake. **Deviation** — the plan assumed spend could be wired "into the existing `Hub.register(...)` start flow", but `register()` only attaches listeners; the wake is what starts a game, and it is synchronous. The gate is unset by default, so with no token layer the wake path is byte-for-byte the old behaviour. `toggleFs()` routes through it too, otherwise the fullscreen button would be a free way to start a paid game <!-- id: 91 -->
- [x] Add `store.observe()` to `src/lib/storage.ts` and submit scores from there rather than editing each game. **Deviation from task id:86's wording** ("wire into each existing game's score-save path"): all twelve games already funnel their personal best through `store.set("best:{game}", n)`, and storage.ts is the only module allowed to touch localStorage (ADR-4), so that is where the score-save paths converge. One integration point instead of twelve, and zero changes to game logic — which is what approach.md's Migrations & Compat section actually requires <!-- id: 92 -->
- [x] Add `src/lib/tokenGames.ts` mapping hub id ↔ localStorage best key ↔ backend slug. Three identifiers exist for each game and none agree. **Flagged**: the sequence game is titled ECHO on the site and seeded as `simon` in the backend — mapping is unambiguous but the names should be reconciled <!-- id: 93 -->
- [x] Add the `/account/` page + Vite input entry and an ACCOUNT link in the hub header. **Deviation**: the Account surface had no page of its own in the plan; the hub has no room for a transaction log and riddles list <!-- id: 94 -->
